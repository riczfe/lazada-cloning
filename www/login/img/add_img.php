		...
		if(isset($_FILES) & !empty($_FILES)){
			...
			if(isset($name) && !empty($name)){
				if(($extension == "jpg" || $extension == "jpeg" || $extension == "png") && ($type == "image/jpeg" || $type == "image/png") && $size<=$max_size){
					$location = "../img_uploads/";
					$move = move_uploaded_file($tmp_name, $location.$name);
					
					if($move){
						$database = json_decode(file_get_contents("../img_data/img.json"), 1);
						if ($database == NULL) {
							$database = array();
							// throw new RuntimeException("Database not found");
						}
						array_push($database, array(
							"image" => "img_uploads/".$name,
							"description" => $_POST['description']
						));
						file_put_contents('../img_data/img.json', json_encode($database));
